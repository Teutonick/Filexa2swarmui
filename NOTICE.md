...
inside the user's local SwarmUI extension data directory.
...
This connector is intended to work with SwarmUI, but it is not part of SwarmUI
and is not endorsed by, sponsored by, or affiliated with the SwarmUI project
unless explicitly stated by that project.

SwarmUI is third-party software. Users install, configure, update, and operate
SwarmUI at their own risk and subject to SwarmUI's own license, documentation,
and terms.

This repository does not include SwarmUI, AI models, model weights, checkpoints,
text encoders, VAEs, ComfyUI nodes, CUDA, GPU drivers, or any other third-party
runtime components unless explicitly stated.
...
The connector runs inside the user's local SwarmUI installation.

The connector is designed to make outbound HTTP/HTTPS requests to the configured
Filexa API endpoint. It does not require exposing the user's local SwarmUI port
...
The connector can cause local image generation tasks to run on the user's
machine when enabled and connected to Filexa. Users are responsible for reviewing
their configuration, model selection, resource usage, and local security before
enabling the connector.

## Data and Privacy

The connector may transmit task data, prompts, generation parameters, status
information, errors, and generated results between the user's local SwarmUI
...
secure, uninterrupted, error-free, compatible with any specific SwarmUI version,
...
The authors and copyright holders are not responsible for SwarmUI, AI models,
...
- installing and operating SwarmUI;
...
arising from or related to the connector, Filexa service integration, SwarmUI,
...
SwarmUI and other third-party names, logos, and trademarks belong to their
