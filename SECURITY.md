...
- GPU drivers, CUDA, Python, .NET, operating systems, or other runtime components;
- Telegram, hosting providers, payment providers, or other external services;
- the user's local computer, local network, or SwarmUI installation;
...
- unintended exposure of the local SwarmUI API to the internet;
...
- problems caused by manually modified SwarmUI installations;
- vulnerabilities in SwarmUI itself;
- vulnerabilities in third-party models or model download sources;
- issues caused by exposing SwarmUI or other local services to the internet;
...
- violate SwarmUI, Telegram, model provider, hosting provider, or other third-party terms.
...
It does not require exposing the user's local SwarmUI port to the public internet.

Users should not open public inbound access to SwarmUI unless they understand the security risks and have configured appropriate authentication, firewalling, and network controls.

## Dependency Security

This project may depend on third-party libraries and the user's local SwarmUI installation.
...
Users install, configure, and run the connector, SwarmUI, third-party models, and local generation software at their own risk.